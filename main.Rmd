...
```{r}
grid = 10^seq(6,-10,length=100)
cv.out.ridge = cv.glmnet(X, z, alpha=0, family = binomial('logit'), nfolds=B, lambda=grid, foldid = foldid)
cv.out.lasso = cv.glmnet(X, z, alpha=1, family = binomial('logit'), nfolds=B, lambda=grid, foldid = foldid)
...
plot(1-beta, alpha, xlim = c(0,1), ylim = c(0,1))
```